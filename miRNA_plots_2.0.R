...
mirna_seed <- read.table("~/Dropbox/temp/miRNA/combined_by_mir_overlap/overlap.snps.named.seed.txt", sep = "\t")
mirna_full <- read.table("~/Dropbox/temp/miRNA/combined_by_mir_overlap/overlap.snps.full.named.txt", sep = "\t")
mirna_ann <- read.table("~/Dropbox/temp/miRNA/combined_by_mir_overlap/overlap.seed_or_full.named.txt", sep = "\t")
...
###SEED OR FULL
ggplot(mirna_ann, aes(x=V32)) + 
  geom_histogram(aes(fill = V34), stat = "count", position = "dodge") +
  theme_bw() +
  # scale_fill_brewer(breaks=c("<= 500", "501 - 1000", "1001 - 1500", "1501 - 2000", "2001 - 2500", "2501 - 3000"), name="Distance from stop codon (bp)", palette="RdPu") + 
  xlab("") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) +
