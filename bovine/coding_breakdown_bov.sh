...
for gene in cgn colec43 colec46 colec10 colec11 colec12 fcn2 masp1 masp2 mbl1 mbl2 sftpa sftpd
	do
		...
		echo -e "$caps_gene"'\t'"synonymous"'\t'"$all_syn"'\t'"$norm_syn"'\t'"$dis_syn" >> coding_type_breakdown.txt
		echo -e "$caps_gene"'\t'"missense"'\t'"$all_mis"'\t'"$norm_mis"'\t'"$dis_mis" >> coding_type_breakdown.txt
done
echo done.